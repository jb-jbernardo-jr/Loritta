	...
	override fun run(context: CommandContext, locale: BaseLocale) {
		...
		val callback: (List<LorittaProfile>) -> Unit = {
			for ((idx, userProfile) in it.withIndex()) {
				...
				val avatar = LorittaUtils.downloadImage(member.user.effectiveAvatarUrl).getScaledInstance(143, 143, BufferedImage.SCALE_SMOOTH)
		...
		loritta.getLorittaProfileForUsers(userIds.toTypedArray(), callback)
	}
}
